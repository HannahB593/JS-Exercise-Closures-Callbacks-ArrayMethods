...
function processSum(numberList, callback) {
    var sum = [a, b].reduce(function(numberList, callback) {
...
function processProduct(num1, num2) {
    var sum = 1;
    for (var i = 0; i < processProduct.length; i++) {
        sum = sum * processProduct[i];
    }
    return sum;
}
...
function getFullNames(array) {
    //added Parameter "array" could have done runners too.
    let newArray = []; //Creaed an empty array awaiting some new data
    array.forEach(array => {
        //this is saying: in the runners array, for each item...
        newArray.push("${array.last_name}, ${array.first_name}"); //this is saying: Push the following data.
        // into this new array I created
    });

    return newArray; // return that new array with the new data I pushed into it.
}
